...
export default function RewardsPage({ user }) {
  ...
  return (
    <div className="p-6 space-y-8 min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-200 transition-colors">

      <h1 className="text-3xl font-bold text-teal-500 flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 transition-colors">
        ...
        <h2 className="text-2xl font-semibold mb-4 text-teal-500 flex items-center gap-2">
          <Trophy /> Leaderboard
        </h2>
        ...
        {leaderboard.length ? (
          <table className="w-full border border-gray-200 dark:border-gray-700 rounded-lg">
            ...
            <thead className="bg-gray-100 dark:bg-gray-900 text-teal-500">
              <tr>
            ...
            <tbody>
              {leaderboard.map((it, i) => (
                <tr
                  key={i}
                  className="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700/40 transition-colors"
                >
                  <td className="p-3">{i + 1}</td>
                  <td className="p-3">{it.employeeName}</td>
                  <td className="p-3 text-teal-500">{it.totalRewards}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {myRewards.length ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {myRewards.map((r) => (
                <div
                  key={r._id}
                  className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors"
                >
      ...
      {user.role === "admin" && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 transition-colors">
          ...
          <select
            ...
            className="p-3 bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg w-full mb-4 text-gray-900 dark:text-white"
          >
          ...
          <button
            onClick={giveReward}
            disabled={saving}
            className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-3 rounded-lg w-full transition"
          >
            {saving ? "Giving..." : "Give Reward"}
          </button>

          {/* Rewards List */}
          <h3 className="text-xl font-semibold text-teal-500 mt-8">
            Rewards Given
          </h3>
          ...
          {allRewards.length ? (
            <ul className="space-y-3 mt-3">
              ...
              {allRewards.map((r) => (
                <li
                  key={r._id}
                  className="p-3 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg transition-colors"
                >
